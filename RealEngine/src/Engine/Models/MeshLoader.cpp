...
namespace RealEngine {
	...
	void MeshLoader::LoadMeshFromOBJ(const std::string& fileName, Mesh& mesh)
	{
	...
	void MeshLoader::LoadMeshFromFBX(const std::string& path, Mesh& myMesh)
	{
	...
	void MeshLoader::LoadNode(FbxNode* node, Mesh& myMesh)
	{
	...
	bool MeshLoader::Initialize(FbxMesh* mesh, Mesh& myMesh)
	{
		if (!mesh->GetNode())
			return false;
	...
	void MeshLoader::ProcessControlPoints(const FbxMesh* mesh, std::vector<glm::vec3>& positions)
	{
	...
	glm::vec2 MeshLoader::ReadUV(const FbxMesh* inMesh, int inCtrlPointIndex, int inTextureUVIndex, int inUVLayer)
	{
	...
	glm::vec3 MeshLoader::ReadNormal(const FbxMesh* mesh, int controlPointIndex, int vertexCounter)
	{
